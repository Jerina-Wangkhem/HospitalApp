...
namespace Hospital.Services
{
    public class HospitalInfoService:IHospitalInfo
    {
        ...
        {
            var model = new HospitalInfoViewModel().ConvertViewModel(hospitalInfo);
            var ModelById = _unitOfWork.GenericRepository<Models.HospitalInfo>().GetById(model.Id);
            ...
            _unitOfWork.GenericRepository<Models.HospitalInfo>().Update(ModelById);
            _unitOfWork.Save();
        }

        private List<HospitalInfoViewModel> ConvertModeltoViewModeList(List<HospitalInfo> modelList)
        {
